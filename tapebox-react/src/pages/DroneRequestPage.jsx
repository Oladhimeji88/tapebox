    ...
    <main className="flex-1 w-full">
      ...
      <section className="max-w-3xl mx-auto p-8 my-12">
        ...
        <div className="bg-white p-8 rounded-lg shadow-md">
          <h2 className="text-2xl font-medium text-green-700 mb-8">Mission Details</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input type="text" required placeholder="Nearest Drone PUD address" className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-green-500" />
            </div>
            <div>
              <label className="block text-gray-700 font-medium mb-2">Delivery Coordinates / Address *</label>
              <input type="text" required placeholder="Destination address or GPS coordinates" className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-green-500" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-gray-700 font-medium mb-2">Package Weight (kg) *</label>
                <input type="number" min="0.1" max="5" step="0.1" required className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-green-500" />
              </div>
              <div>
                <label className="block text-gray-700 font-medium mb-2">Preferred Mission Time *</label>
                <input type="datetime-local" required className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-green-500" />
              </div>
            </div>
            <div>
              <label className="block text-gray-700 font-medium mb-2">Package Contents *</label>
              <textarea rows="3" required placeholder="Describe the contents" className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-green-500" />
            </div>
            <button type="submit" className="w-full bg-green-600 text-white py-3 rounded hover:bg-green-700 transition font-medium">
